...
//fonction permettant de savoir si un utilisateur est bien un manager : on compare son id à l'id lorsque allow = 1

function isManager(){
  $bdd = new PDO('mysql:host=localhost;dbname=infinite_;charset=utf8', 'root', '');

  try {
      $reqManager = $bdd->query("SELECT * FROM user WHERE allow = 1");
  
      while ($manager=$reqManager->fetch()){
          if($_SESSION['id']==$manager['idUser']){
              return true;
          }
      }
